"""Integration gateway to the ECG system. Same MPI-lookup + normalize shape as
backend/lab_gateway.py and backend/mri_gateway.py."""
import asyncio
from data import ecg_system
...
def _normalize(row: dict, patient_id: str) -> dict:
    return {
        ...
        "result": row["result"],
        "doctor": row["doctor"],
        "report_image": row["report_image"] or None,
    }
...
async def get_records_for_patient(db, patient_id: str) -> list[dict]:
    ...
    if not mpi_row:
        return []
    rows = await asyncio.to_thread(ecg_system.query_by_local_id, mpi_row["ecg_local_id"])
...
async def get_all_records(db) -> list[dict]:
    ...
    local_to_canonical = {m["ecg_local_id"]: m["patient_id"] for m in mpi_rows}
    rows = await asyncio.to_thread(ecg_system.query_all)
    records = []
    for r in rows:

"""Integration gateway to the X-Ray system. Same MPI-lookup + normalize shape as
backend/lab_gateway.py and backend/mri_gateway.py."""
import asyncio
from data import xray_system
...
async def get_records_for_patient(db, patient_id: str) -> list[dict]:
    ...
    if not mpi_row:
        return []
    rows = await asyncio.to_thread(xray_system.query_by_local_id, mpi_row["xray_local_id"])
...
async def get_all_records(db) -> list[dict]:
    ...
    local_to_canonical = {m["xray_local_id"]: m["patient_id"] for m in mpi_rows}
    rows = await asyncio.to_thread(xray_system.query_all)
    records = []
    for r in rows:
